...
Athleo Mobile is a React Native (0.81) + Expo (SDK 54) app for syncing HealthKit / Health Connect data to the Athleo backend and displaying daily nutrition, exercise, hydration, and related health summaries.

This file is the package guide for `AthleoMobile/`. Use it for work in this directory. This app lives in a monorepo, so if a task crosses into the backend, frontend, or `shared/`, read the matching guide there before editing outside this package.
...
Work from `AthleoMobile/` for implementation and validation. Do not treat the monorepo root as the mobile app entrypoint.

`tsconfig.json` defines:
- `@/*` for local package imports
- `@workspace/shared` for `../shared/src/index.ts`

The app talks to the Athleo backend primarily under `/api` and sends health sync payloads to `POST /api/health-data`.
...
- For work inside `AthleoMobile/`, this file is the package guide.
