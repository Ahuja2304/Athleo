...
Athleo is a comprehensive fitness tracking application built with React 18 + TypeScript + Vite frontend and Node.js/Express backend, using PostgreSQL with Row Level Security.
...
- **Docker & Docker Compose**: Essential for running Athleo in containerized environments.
- **Git**: For cloning the Athleo repository.
...
The fastest way to get Athleo running is using our Docker helper script:

```bash
# Clone the repository
git clone https://github.com/Ahuja2304/Athleo.git
cd Athleo
...
2. **Create database**:
   ...
   GRANT ALL PRIVILEGES ON DATABASE athleo TO sparky;
   ```
...
- `ATHLEO_SERVER_HOST` - Backend service connection
- `ATHLEO_SERVER_PORT` - Backend service port
...
- **Linting**: Run `npm run lint` in the `AthleoFrontend` directory to check for potential errors and enforce coding style.
- **Formatting**: Run `npx prettier . --check` in the `AthleoFrontend` directory to verify consistent code formatting. If you need to fix formatting issues, use `npm run format`.
