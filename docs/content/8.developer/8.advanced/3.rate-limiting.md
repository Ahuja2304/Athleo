...
This document outlines the rate limiting strategy implemented for the Athleo application, focusing on protecting sensitive authentication endpoints.
...
location = /api/auth/login {
    ...
    proxy_pass http://${ATHLEO_SERVER_HOST}:${ATHLEO_SERVER_PORT}/auth/login;
...
location = /api/auth/register {
    limit_req zone=auth_zone burst=5 nodelay;
    proxy_pass http://${ATHLEO_SERVER_HOST}:${ATHLEO_SERVER_PORT}/auth/register;
    # ... other proxy settings ...
}
```
...
location ~ ^/api/auth/mfa(/.*)?$ {
    limit_req zone=auth_zone burst=5 nodelay;
    rewrite ^/api/(.*)$ /$1 break;
    proxy_pass http://${ATHLEO_SERVER_HOST}:${ATHLEO_SERVER_PORT};
    # ... other proxy settings ...
}
```
...
|----------|-------------|---------|
| `ATHLEO_API_KEY_RATELIMIT_WINDOW_MS` | Time window in milliseconds | `60000` |
| `ATHLEO_API_KEY_RATELIMIT_MAX_REQUESTS` | Max requests per window | `100` |
