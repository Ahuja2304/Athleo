...
*   **Exercise Search Component**: This component will query the Athleo Backend API for exercise search results.
*   **Athleo Backend API**: This acts as a central point, routing requests to the appropriate services. It will handle authentication and authorization.
...
*   **New Wger Exercise Service**: This service will be responsible for:
    ...
    *   Mapping `wger` data to Athleo's `public.exercises` schema.
...
*   **Athleo DB: User Exercises**: The existing `public.exercises` table, which will store user-specific exercises, including those imported from `wger`.
