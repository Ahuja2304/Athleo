...
This comprehensive guide provides solutions to common issues and debugging techniques for Athleo developers.

## Common Issues and Solutions

### Port Conflicts

If you encounter errors indicating that a port is already in use, another application is likely using one of the ports required by Athleo (e.g., 8080 for frontend, 3010 for backend, 5432 for PostgreSQL).
...
1. **Check database logs**:
   ```bash
   ./docker/docker-helper.sh dev logs athleo-db
   ```
2. **Verify `.env` settings**: Ensure `ATHLEO_DB_NAME`, `ATHLEO_DB_USER`, and `ATHLEO_DB_PASSWORD` are correct.
...
This error typically indicates that your `ATHLEO_API_ENCRYPTION_KEY` in your `.env` file is not correctly configured. Ensure it is a 64-character hexadecimal string.
...
Athleo implements rate limiting at the Nginx reverse proxy layer to prevent brute-force attacks and DoS attempts.
...
```bash
./docker/docker-helper.sh dev logs -f athleo-frontend
./docker/docker-helper.sh dev logs -f athleo-server
./docker/docker-helper.sh dev logs -f athleo-db
```
...
docker exec -it athleo-server-1 tail -f /app/logs/app.log
```
...
```bash
# Frontend container
docker exec -it athleo-frontend-1 sh

# Backend container
docker exec -it athleo-server-1 sh

# Database container
docker exec -it athleo-db-1 psql -U sparky -d athleo_db
```
...
docker inspect athleo-server-1
```
...
docker exec -it athleo-server-1 npm run migrate:status

# Force migration reset (DANGEROUS)
docker exec -it athleo-server-1 npm run migrate:reset
```
...
docker exec -it athleo-frontend-1 ping athleo-server
```
...
docker exec -it athleo-server-1 env | grep SPARKY

# Verify frontend environment variables
docker exec -it athleo-frontend-1 env | grep VITE
```
...
docker exec -it athleo-frontend-1 npm run type-check
...
docker exec -it athleo-server-1 node --inspect=0.0.0.0:9229 server.js
```
...
docker scan athleo-server:latest
```
