# Internationalization (i18n) Setup in AthleoFrontend

This document outlines the setup for internationalization (i18n) in the AthleoFrontend application using `i18next` and `react-i18next`.
...
These dependencies are installed in the `AthleoFrontend` directory.
...
The `i18next` instance is configured in [`AthleoFrontend/src/i18n.ts`](AthleoFrontend/src/i18n.ts).
...
The `i18next` instance is initialized and provided to the React application in [`AthleoFrontend/src/main.tsx`](AthleoFrontend/src/main.tsx).
...
A dedicated component, [`AthleoFrontend/src/components/LanguageHandler.tsx`](AthleoFrontend/src/components/LanguageHandler.tsx), is used to synchronize the `i18next` language with the user's preference stored in the `PreferencesContext`.
...
3.  **Update `i18n.ts`:** Add `'es'` to the `supportedLngs` array in [`AthleoFrontend/src/i18n.ts`](AthleoFrontend/src/i18n.ts).
4.  **Update Language Switcher:** Add a new `SelectItem` for Spanish in the language switcher within [`AthleoFrontend/src/components/Settings.tsx`](AthleoFrontend/src/components/Settings.tsx).
