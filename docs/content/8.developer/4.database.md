...
This document covers the database structure, schema design, and migration process for Athleo.

## Database Overview

Athleo uses PostgreSQL 15+ with Row Level Security (RLS) to ensure data isolation between users. The database follows a normalized design with clear relationships between entities.
...
Athleo uses a custom migration system that runs automatically on server startup.
...
Migrations are stored in `AthleoServer/db/migrations/` with the naming pattern:
```
...
This database design provides a robust foundation for Athleo, ensuring data integrity, security, and performance while supporting all application features.
