...
This page provides instructions for installing and running Athleo using Docker Compose. This method is recommended for most users as it simplifies the setup process.
...
1.  **Create a new folder and download Docker files**:
    Create a new directory for Athleo and navigate into it. Then, download the necessary `docker-compose.yml` and `.env` files.
    ```bash
    mkdir athleo && cd athleo
    curl -o docker-compose.yml https://github.com/Ahuja2304/Athleo/releases/latest/download/docker-compose.prod.yml    
...
3.  **Start the Application**:
    From within the `athleo` directory (where `docker-compose.yml` is located), pull the latest images and start the application services:
    ```bash
...
*   **`athleo-server`**:
    *   **Image**: `codewithcj/athleo_server:latest`
    ...
    *   **Dependencies**: Depends on `athleo-db` to ensure the database is running before the server starts.

*   **`athleo-frontend`**:
    *   **Image**: `codewithcj/athleo:latest`
    ...
    *   **Dependencies**: Depends on `athleo-server` to ensure the backend is running.

## Accessing the Application

Once all services are up and running, you can access the Athleo frontend in your web browser at the URL you configured for `ATHLEO_FRONTEND_URL` (e.g., `http://localhost:3004`).
