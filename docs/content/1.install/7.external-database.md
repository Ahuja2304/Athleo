...
1.  **ATHLEO_DB_USER**: Used for migrations and schema management.
2.  **ATHLEO_APP_DB_USER**: Used by the application components with restricted permissions.
...
CREATE DATABASE athleo_db;

-- 2. Create the DB Owner user (referenced in .env as ATHLEO_DB_USER)
CREATE USER sparky_admin WITH PASSWORD 'your_secure_password';

-- 3. Grant database ownership
ALTER DATABASE athleo_db OWNER TO sparky_admin;
...
```bash
ATHLEO_DB_HOST=your-db-host
ATHLEO_DB_NAME=athleo_db
ATHLEO_DB_USER=sparky_admin
ATHLEO_DB_PASSWORD=your_secure_password
ATHLEO_DB_PORT=5432
```

> [!IMPORTANT]
> The application will automatically create the restricted **App User** (defined in `ATHLEO_APP_DB_USER`) and grant it necessary permissions during the first startup, provided the `ATHLEO_DB_USER` has the `CREATEROLE` privilege.
