...
This guide provides a step-by-step tutorial for upgrading your Athleo PostgreSQL database from version 15-alpine to 18.3-alpine.
> [!IMPORTANT]
...
## Prerequisites
- Athleo installed via Docker Compose.
...
## Default Paths
This guide assumes you are using the default Athleo directory structure. If you have custom paths, replace
`./postgresql` with your actual path.
## Assumptions
- **Base path**: Your Athleo installation directory (where `docker-compose.yml` is located).
- **Default Database Path**: `./postgresql` (relative to the compose file).
- **Database Username**: `sparky` (Verify this in your `.env` file. This is your ATHLEO_DB_USER).
- **Database Name: `athleo_db` (this is your ATHLEO_DB_NAME. default is athleo_db)
---
## Part 1: Backup and Shutdown
### 1. Perform a Full Data Backup
Run this on your host terminal to export all your data into a single SQL file. Replace `[OLD_CONTAINER_NAME]` with your actual container name (e.g., `athleo-db` or `postgres15-alpine`):
run "docker ps " to find the old container name. in the below command sparky is your ATHLEO_DB_USER
```bash
...
Modify the `athleo-db` service to use the new image and the corrected mount point structure required for version 18.3.
We are updating the container name to athleo-db and image to postgres:18.3
```yaml
version: "3.9"
services:
  athleo-db:
    image: postgres:18.3-alpine
    container_name: athleo-db
```
### 4. Update your `docker-compose.yml`
...
```bash
cat ./full_backup.sql | docker exec -i athleo-db psql -U sparky
```
if a restore fails try this
```bash
cat ./full_backup.sql | docker exec -i athleo-db psql -U sparky -d athleo_db
```
