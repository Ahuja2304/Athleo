...
const PORT = process.env.ATHLEO_SERVER_PORT || 3010;

console.log(
  `DEBUG: ATHLEO_FRONTEND_URL is: ${process.env.ATHLEO_FRONTEND_URL}`,
);
...
app.use(
  cors({
    origin: createCorsOriginChecker(
      process.env.ATHLEO_FRONTEND_URL || "http://localhost:8080",
      allowPrivateNetworks,
      process.env.ATHLEO_EXTRA_TRUSTED_ORIGINS,
    ),
...
console.log("AthleoServer UPLOADS_BASE_DIR:", UPLOADS_BASE_DIR);
...
applyMigrations()
  ...
  .then(async () => {
    // Upsert OIDC provider from env when ATHLEO_OIDC_ISSUER_URL + CLIENT_ID + SECRET + PROVIDER_SLUG are set
    try {
    ...
    if (process.env.ATHLEO_ADMIN_EMAIL) {
      const userRepository = require("./models/userRepository");
      const adminUser = await userRepository.findUserByEmail(
        process.env.ATHLEO_ADMIN_EMAIL,
      );
    ...
    const server = app.listen(PORT, () => {
      ...
      log("info", `AthleoServer listening on port ${PORT}`);
