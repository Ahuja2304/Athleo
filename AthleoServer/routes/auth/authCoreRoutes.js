...
router.get('/settings', async (req, res) => {
    try {
        ...
        if (process.env.ATHLEO_FRONTEND_URL) {
            try {
                ...
                log('warn', `[AUTH CORE] Invalid frontend URL for trusted origin: ${process.env.ATHLEO_FRONTEND_URL}`);
            }
        }

        // Environment overrides are now handled within globalSettingsRepository.getGlobalSettings()
        const oidcAutoRedirectEnv = process.env.ATHLEO_OIDC_AUTO_REDIRECT === 'true';
        ...
        const forceEmailLogin = process.env.ATHLEO_FORCE_EMAIL_LOGIN === 'true';
        const disableEmailLogin = process.env.ATHLEO_DISABLE_EMAIL_LOGIN === 'true';
        res.json({
            trusted_origin: null,
            email: { enabled: forceEmailLogin || !disableEmailLogin },
            oidc: { enabled: process.env.ATHLEO_OIDC_AUTH_ENABLED === 'true', providers: [], auto_redirect: false }
        });
    }
});
