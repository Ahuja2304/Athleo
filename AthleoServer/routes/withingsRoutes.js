// AthleoServer/routes/withingsRoutes.js
...
router.get('/authorize', authMiddleware.authenticate, async (req, res) => {
    try {
        ...
        const baseUrl = process.env.ATHLEO_FRONTEND_URL || 'http://localhost:8080';
...
router.post('/callback', async (req, res) => {
    try {
        ...
        const tokenExchangeResult = await withingsService.exchangeCodeForTokens(userId, code, `${process.env.ATHLEO_FRONTEND_URL}/withings/callback`, state);
