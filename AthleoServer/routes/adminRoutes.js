...
// This will be enhanced later to prioritize ATHLEO_ADMIN_EMAIL
...
router.delete('/users/:userId', async (req, res, next) => {
  try {
    ...
    if (user.email === process.env.ATHLEO_ADMIN_EMAIL) {
...
router.put('/users/:userId/role', async (req, res, next) => {
  try {
    ...
    if (user.email === process.env.ATHLEO_ADMIN_EMAIL && role !== 'admin') {
...
router.post('/users/:userId/reset-password', async (req, res, next) => {
  try {
    ...
    await auth.api.forgotPassword({
      email: user.email,
      redirectTo: (process.env.ATHLEO_FRONTEND_URL || 'http://localhost:8080') + '/reset-password'
    });
