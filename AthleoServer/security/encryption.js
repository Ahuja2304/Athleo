...
const ENCRYPTION_KEY_RAW = process.env.ATHLEO_API_ENCRYPTION_KEY;

if (!ENCRYPTION_KEY_RAW) {
  log(
    "error",
    "Encryption key (ATHLEO_API_ENCRYPTION_KEY) is missing. Update your environment variable and try again.",
  );
  process.exit(1);
}
...
if (ENCRYPTION_KEY_RAW.length !== 64 && ENCRYPTION_KEY_RAW.length !== 44) {
  log(
    "error",
    `Encryption key (ATHLEO_API_ENCRYPTION_KEY) has an invalid length (${ENCRYPTION_KEY_RAW.length}). Expected 64 hex characters or 44 Base64 characters.`,
  );
  process.exit(1);
}
