...
Athleo Server is the Node.js + Express 5 backend API for the Athleo monorepo. This file is the package guide for `AthleoServer/`.
...
- Runtime entrypoint: `AthleoServer.js`
...
- `AthleoServer.js` - app entrypoint, route registration, Swagger mounting, cron setup, graceful shutdown
...
- `AthleoServer.js` is the single app entrypoint. Do not create alternate boot paths unless the task explicitly calls for one.
- Route mounting is centralized in `AthleoServer.js`. If you add a new router, wire it there.
...
- Commonly required environment variables include:
  ...
  - `ATHLEO_DB_PASSWORD`
  - `ATHLEO_APP_DB_USER`
  - `ATHLEO_APP_DB_PASSWORD`
  - `ATHLEO_FRONTEND_URL`
  - `JWT_SECRET`
  - `ATHLEO_API_ENCRYPTION_KEY`
- Common operational toggles include:
  - `ATHLEO_SERVER_PORT`
  - `ATHLEO_LOG_LEVEL`
  - `ATHLEO_ADMIN_EMAIL`
  - `ALLOW_PRIVATE_NETWORK_CORS`
  - `ATHLEO_EXTRA_TRUSTED_ORIGINS`
...
- Public API change:
  inspect the affected route file, then confirm registration in `AthleoServer.js` and Swagger coverage
- Startup, env, or deployment issue:
  inspect `AthleoServer.js`, `utils/preflightChecks.js`, `utils/secretLoader.js`, and `config/logging.js`
...
- For work inside `AthleoServer/`, this file is the package guide.
