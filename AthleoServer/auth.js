...
console.log("DEBUG: Initializing Better Auth Pool with:", {
    user: process.env.ATHLEO_DB_USER,
    host: process.env.ATHLEO_DB_HOST,
    database: process.env.ATHLEO_DB_NAME,
    port: process.env.ATHLEO_DB_PORT || 5432,
    password: process.env.ATHLEO_DB_PASSWORD ? "****" : "MISSING"
});
*/
...
const authPool = new Pool({
  ...
  host: process.env.ATHLEO_DB_HOST,
  database: process.env.ATHLEO_DB_NAME,
  password: process.env.ATHLEO_DB_PASSWORD,
  port: process.env.ATHLEO_DB_PORT || 5432,
});
...
// Initial sync on startup - deferred to AthleoServer.js after migrations
...
const apiKeyPlugin = require("@better-auth/api-key").apiKey({
  ...
  rateLimit: {
    ...
    timeWindow:
      Number.parseInt(
        process.env.ATHLEO_API_KEY_RATELIMIT_WINDOW_MS,
        10,
      ) || 60_000, // 1 minute
    maxRequests:
      Number.parseInt(
        process.env.ATHLEO_API_KEY_RATELIMIT_MAX_REQUESTS,
        10,
...
const auth = betterAuth({
  ...
  baseURL:
    (process.env.ATHLEO_FRONTEND_URL?.startsWith("http")
      ? process.env.ATHLEO_FRONTEND_URL
      : `https://${process.env.ATHLEO_FRONTEND_URL}`
    )?.replace(/\/$/, "") + "/api/auth",
  ...
  emailAndPassword: {
    enabled: process.env.ATHLEO_DISABLE_EMAIL_LOGIN !== "true",
  ...
  advanced: {
    ...
    useSecureCookies:
      process.env.ALLOW_PRIVATE_NETWORK_CORS === "true"
        ? false
        : process.env.ATHLEO_FRONTEND_URL?.startsWith("https"),
    trustProxy: true,
  ...
  trustedOrigins: (() => {
    const origins = [process.env.ATHLEO_FRONTEND_URL];
    ...
    if (process.env.ALLOW_PRIVATE_NETWORK_CORS === "true") {
      ...
      if (process.env.ATHLEO_EXTRA_TRUSTED_ORIGINS) {
        const extras = process.env.ATHLEO_EXTRA_TRUSTED_ORIGINS.split(
          ",",
        ).map((o) => o.trim());
        origins.push(...extras);
      }
    ...
    user: {
      create: {
        before: async (user, ctx) => {
          ...
          if (process.env.ATHLEO_DISABLE_SIGNUP === "true") {
            console.log(
              "[AUTH] Blocking signup: ATHLEO_DISABLE_SIGNUP is true",
            );
