// AthleoServer/integrations/withings/withingsService.js
...
async function getAuthorizationUrl(userId, redirectUri) {
  ...
  try {
    ...
    return `${WITHINGS_ACCOUNT_BASE_URL}/oauth2_user/authorize2?response_type=code&client_id=${clientId}&scope=${scope}&redirect_uri=${process.env.ATHLEO_FRONTEND_URL}/withings/callback&state=${state}`;
  } finally {
    client.release();
  }
}
