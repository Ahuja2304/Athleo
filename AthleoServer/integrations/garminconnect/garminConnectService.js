...
async function handleGarminTokens(userId, tokensB64) {
    try {
        ...
        if (error.message.includes('Invalid key length')) {
            errorMessage = `Failed to handle Garmin tokens: Encryption key (ATHLEO_API_ENCRYPTION_KEY) has an invalid length. Expected 64 hex characters or 44 Base64 characters. Update your environment variable and try again.`;
        }
