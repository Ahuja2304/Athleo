// AthleoServer/integrations/fitbit/fitbitDataProcessor.js
...
async function processFitbitSleep(
  ...
  for (const entry of data.sleep) {
    ...
    if (result && result.id && entry.levels) {
      ...
      // Map Fitbit levels to Athleo supported stages
      // Note: 'awake' is required by Athleo analytics to correctly exclude from 'time asleep'
