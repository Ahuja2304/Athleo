// AthleoServer/services/stravaService.js
...
const STRAVA_DATA_SOURCE =
  process.env.ATHLEO_STRAVA_DATA_SOURCE || "strava";
log(
  "info",
...
async function syncStravaData(
  ...
  log(
    "info",
    `[stravaService] Starting Strava sync (${syncType}) for user ${userId} from ${startDate} to ${endDate}. ENV_SAVE_MOCK_DATA=${process.env.ATHLEO_SAVE_MOCK_DATA}`,
  );
  ...
  if (STRAVA_DATA_SOURCE === "local") {
    ...
    if (!bundle || !bundle.responses) {
      throw new Error(
        'Raw diagnostic bundle not found. Please run a sync with ATHLEO_STRAVA_DATA_SOURCE unset (or set to "strava") ' +
