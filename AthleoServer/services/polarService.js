// AthleoServer/services/polarService.js
...
const POLAR_DATA_SOURCE =
  process.env.ATHLEO_POLAR_DATA_SOURCE || "polar";
log(
  "info",
...
async function syncPolarData(
  ...
  log(
    "info",
    `[polarService] Starting Polar sync (${syncType}) for user ${userId}${providerId ? ` (Provider ID: ${providerId})` : ""}${startDate ? ` from ${startDate}` : ""}${endDate ? ` to ${endDate}` : ""}. ENV_SAVE_MOCK_DATA=${process.env.ATHLEO_SAVE_MOCK_DATA}`,
  );
  ...
  if (POLAR_DATA_SOURCE === "local") {
    ...
    if (!bundle || !bundle.responses) {
      throw new Error(
        'Raw diagnostic bundle not found. Please run a sync with ATHLEO_POLAR_DATA_SOURCE unset (or set to "polar") ' +
          "and ATHLEO_SAVE_MOCK_DATA=true to capture raw API responses first.",
      );
    }
