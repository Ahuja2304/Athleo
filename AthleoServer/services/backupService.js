...
// const { configureSessionMiddleware } = require('../AthleoServer'); // Removed to fix circular dependency
...
async function performBackup(isManual = false) {
  ...
  const dbBackupFileName = `athleo_db_backup_${timestamp}.sql.gz`;
  const uploadsBackupFileName = `athleo_uploads_backup_${timestamp}.tar.gz`;
  const fullBackupFileName = `athleo_full_backup_${timestamp}.tar.gz`;
  ...
  try {
    ...
    const pgDumpArgs = [
      '-h', process.env.ATHLEO_DB_HOST,
      '-p', process.env.ATHLEO_DB_PORT,
      '-U', process.env.ATHLEO_DB_USER,
      '-d', process.env.ATHLEO_DB_NAME,
    ];
    const pgDump = spawn('pg_dump', pgDumpArgs, {
      env: { PGPASSWORD: process.env.ATHLEO_DB_PASSWORD, ...process.env },
    });
...
async function applyRetentionPolicy() {
  ...
  for (const file of files) {
    if (file.startsWith('athleo_full_backup_') && file.endsWith('.tar.gz')) {
...
async function performRestore(backupFilePath) {
  ...
  try {
    ...
    const dbDumpFile = extractedFiles.find(f => f.startsWith('athleo_db_backup_') && f.endsWith('.sql.gz'));
    const uploadsTarFile = extractedFiles.find(f => f.startsWith('athleo_uploads_backup_') && f.endsWith('.tar.gz'));
    ...
    const terminateConnectionsCommand = `SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '${process.env.ATHLEO_DB_NAME}' AND pid <> pg_backend_pid();`;
    await executeCommand(`psql -h ${process.env.ATHLEO_DB_HOST} -p ${process.env.ATHLEO_DB_PORT} -U ${process.env.ATHLEO_DB_USER} -d postgres -c "${terminateConnectionsCommand}"`, { env: { PGPASSWORD: process.env.ATHLEO_DB_PASSWORD, ...process.env } });
    log('info', 'Terminated active database connections.');

    // Drop and recreate database to ensure a clean state
    const dbEnv = { PGPASSWORD: process.env.ATHLEO_DB_PASSWORD, ...process.env };
    const dropDbCommand = `dropdb -h ${process.env.ATHLEO_DB_HOST} -p ${process.env.ATHLEO_DB_PORT} -U ${process.env.ATHLEO_DB_USER} ${process.env.ATHLEO_DB_NAME}`;
    const createDbCommand = `createdb -h ${process.env.ATHLEO_DB_HOST} -p ${process.env.ATHLEO_DB_PORT} -U ${process.env.ATHLEO_DB_USER} ${process.env.ATHLEO_DB_NAME}`;
    ...
    // const { configureSessionMiddleware } = require('../AthleoServer');
