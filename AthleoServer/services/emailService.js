...
// Example using SMTP:
const transporter = nodemailer.createTransport({
  host: process.env.ATHLEO_EMAIL_HOST, // e.g., 'smtp.sendgrid.net'
  port: process.env.ATHLEO_EMAIL_PORT, // e.g., 587 or 465
  secure: process.env.ATHLEO_EMAIL_SECURE === 'true', // true for 465, false for other ports
  auth: {
    user: process.env.ATHLEO_EMAIL_USER, // Your SMTP username
    pass: process.env.ATHLEO_EMAIL_PASS, // Your SMTP password
  },
});
...
async function sendPasswordResetEmail(toEmail, resetUrl) {
  ...
  if (!transporter.options.host || !transporter.options.auth.user) {
    ...
    console.log(`
      ...
      To: ${toEmail}
      Subject: Athleo Password Reset
      
      You have requested a password reset for your Athleo account.
  ...
  try {
    await transporter.sendMail({
      from: process.env.ATHLEO_EMAIL_FROM || 'noreply@athleo.com', // Your sender email address
      to: toEmail,
      subject: 'Athleo Password Reset',
      html: `
        <p>You have requested a password reset for your Athleo account.</p>
...
async function sendEmailMfaCode(toEmail, code) {
  ...
  try {
    await transporter.sendMail({
      from: process.env.ATHLEO_EMAIL_FROM || 'noreply@athleo.com',
      to: toEmail,
      subject: 'Your Athleo MFA Code',
      html: `
...
async function sendMagicLinkEmail(toEmail, magicLinkUrl) {
  ...
  if (!transporter.options.host || !transporter.options.auth.user) {
    ...
    console.log(`
      ...
      To: ${toEmail}
      Subject: Your Athleo Login Link
      
      You have requested a passwordless login to your Athleo account.
  ...
  try {
    await transporter.sendMail({
      ...
      subject: 'Your Athleo Login Link',
      html: `
        <p>You have requested a passwordless login to your Athleo account.</p>
